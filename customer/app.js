...
function restaurants() {
    if (localStorage.getItem('uid')) {
        rest = firebase.database().ref('Restaurants').on('value', (data) => {
            ...
            <img src="https://images.unsplash.com/photo-1606787620651-3f8e15e00662?ixid=MnwxMjA3fDF8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&ixlib=rb-1.2.1&auto=format&fit=crop&w=334&q=80" class="card-img-top" alt="..." id="foodImg">
        <div class="card-body">
            <h2 class="card-title" id="resName">${x.restaurant}</h2>
            <h4 class="card-text">Location : <span id="resLocation"> ${x.country}</span></h4>
            <button class="btn btn-primary" onclick='restaurantItems("${key}")'>visit</button>
        </div>`
            }
        })
    }
    ...
    dish = firebase.database().ref('Restaurants').child(resUid).child('Dishes').on('value', (data) => {
        ...
        for (var key in a) {
            ...
            <div class="card-body">
                ...
                <button onclick='placeOrder("${resUid}","${key}")' class='btn'>Order</button>
            </div>
        </div>`
        }
    })
